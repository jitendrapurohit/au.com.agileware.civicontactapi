...
CREATE TABLE IF NOT EXISTS `civicrm_cca_groups_log` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `groupid` INT UNSIGNED COMMENT 'FK to Group',
  `action` VARCHAR(10) NOT NULL,
  `createdat` timestamp DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT FK_civicrm_cca_groups_log_group_id FOREIGN KEY (`groupid`) REFERENCES `civicrm_group`(`id`) ON DELETE CASCADE
);
